...
- [`SETUP.md`](SETUP.md) — the Firebase configuration only the project owner can do.
...
- **`firebase_options.dart` is not imported yet.** It does not exist until
  `flutterfire configure` runs, so `main.dart` calls `Firebase.initializeApp()`
  bare. Adding the import and passing `DefaultFirebaseOptions.currentPlatform`
  is a two-line change, listed in [SETUP.md](SETUP.md).
